...
def gen_one_nvidia(
    ...
    *,
    ...
    """Generate via Pollinations.ai (free, no key). Uses FLUX model.
    Returns raw JPEG bytes transcoded to PNG for pipeline consistency."""
    body = (prompt
            .replace("{sci_name}", sci)
            .replace("{com_name}", com)
            .replace("{pose}", POSES[pose]))
    ...
    # Transcode to PNG for pipeline consistency.
    try:
        from PIL import Image
        from io import BytesIO
...
def main() -> int:
    ...
    ap.add_argument("--nvidia-key", help="NVIDIA NIM API key (or NVIDIA_API_KEY env)")
    ap.add_argument("--missing-only", action="store_true",
                    help="Only generate for species without existing illustrations")
    ...
    if args.backend == "gemini" and not gemini_key:
        print("error: GEMINI_API_KEY required for gemini backend (--gemini-key or env)", file=sys.stderr)
        return 2
    if args.backend == "nvidia":
        print("[backend] nvidia (Pollinations.ai / FLUX, no key needed)")
    ...
    # For NVIDIA backend, use the text-only prompt (no reference images).
    # For Gemini, use the default prompt (supports reference images).
    if args.backend == "nvidia":
        nvidia_prompt_path = args.prompt.parent / "prompt-nvidia.template.md"
        if nvidia_prompt_path.exists():
            prompt = load_prompt(nvidia_prompt_path)
        else:
            prompt = load_prompt(args.prompt)
    else:
        prompt = load_prompt(args.prompt)

    # --missing-only: skip species that already have both poses illustrated.
    if args.missing_only:
        before = len(species)
        species = [
            (sci, com) for sci, com in species
            if not (args.out / f"{slugify(sci)}.png").exists()
            or not (args.out / f"{slugify(sci)}-2.png").exists()
        ]
        print(f"[missing-only] {before} species -> {len(species)} missing illustrations")
    ...
    for idx, (sci, com) in enumerate(species):
        slug = slugify(sci)
        pos_ref = None
        if not args.no_refs and args.backend == "gemini":
        ...
        for pose in args.poses:
            ...
            try:
                if args.backend == "nvidia":
                    data = gen_one_nvidia("", prompt, sci, com, pose,
