...
if [ -n "$json" ] && [ "$(echo "$json" | jq -r .status)" = "success" ]; then
  ...
  echo -e "\033[33mCouldn't set latitude and longitude automatically, defaulting to central UK.\033[0m"
  echo -e "\033[33mYou can change this from the web interface: Tools -> Settings -> Location.\033[0m"
  LATITUDE=51.5000
  LONGITUDE=-0.1250
fi
...
################################################################################
#                  Configuration settings for AvianVisitors-UK                 #
################################################################################

# Optional: Site Title for banner

SITE_NAME="AvianVisitors-UK"
...
## can generate one on the fly via free.ai + rembg. Generation itself is a
## cloud call (works on any board); only rembg runs locally, and it is
## skipped automatically when free RAM is low. Keep this enabled everywhere.

GENERATE_ILLUSTRATIONS=1
